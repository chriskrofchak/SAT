...
fn read_lr1(lr1: &String) -> Vec<HashMap<String, ReducePair>> {
    ...
    for (idx, line) in lines.enumerate() {
        match line {
            ...
            Err(_) => {
                panic!("Error reading {} at line no. {}", lr1, idx)
            }
        }
    }
    lr_vec
...
fn reduce(a: &String, state: &usize, dfa_vec: &Vec<HashMap<String, ReducePair>>) -> bool {
    if !dfa_vec[*state].contains_key(&*a) {
        return false;
    }
...
pub fn parse(stream: &mut Vec<Token>, grammar: &String, lr1: &String) -> ParseNode {
    // returns iterator
    let grammar_lines: Vec<Vec<String>> = read_lines(grammar)
        .map(|x| {
            x.unwrap()
                .split_whitespace()
                .map(|y| String::from(y))
    ...
    for Token(lexeme, kind) in toks.iter() {
        ...
        while reduce(&kind.to_string(), &state_top(&state_stack), &dfa_vec) {
        ...
        // push top character
        sym_stack.push(ParseNode(
            Term(Token(lexeme.clone(), kind.clone())),
            Vec::new(),
        ));
        match dfa_vec[state_top(&state_stack)].get(&kind.to_string()) {
            None => {
                panic!("Error parsing at: {}", lexeme);
            }
            Some(red_pair) => {
                state_stack.push(red_pair.1);
            }
        }
    } // end for
    ...
    for item in first_rule.iter().rev() {
        ...
        if *item == first_rule[0] {
            ...
            match sym_stack.pop() {
                None => {
                    panic!(
                        "Error on final derivation, item is {} but stack is empty.",
                        item
                    );
                }
                Some(node) => {
